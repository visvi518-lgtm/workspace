...
class MainWindow(ctk.CTk):
    ...
    def __init__(self, db, portfolio, naver, yahoo, strategy):
        ...
        self.auto_trader = AutoTrader(
            db=db, portfolio=portfolio, strategy=strategy,
            naver=naver, yahoo=yahoo,
    ...
    def _build_header(self):
        hdr = ctk.CTkFrame(self, height=50, fg_color=("#16213e", "#0d0d1a"), corner_radius=0)
        hdr.pack(fill="x")
        hdr.pack_propagate(False)

        ctk.CTkLabel(hdr, text="  📈  모의주식 자동매매",
                     font=("Malgun Gothic", 16, "bold"),
                     text_color="#90caf9").pack(side="left", padx=15)

        self.market_lbl = ctk.CTkLabel(hdr, text="",
                                        font=("Malgun Gothic", 11), text_color="gray")
        self.market_lbl.pack(side="right", padx=20)
    ...
    def _update_market_status(self):
        ...
        self.market_lbl.configure(text=f"{kr}   {us}")

    def _on_trade(self, trade_type, name, price, reason):
